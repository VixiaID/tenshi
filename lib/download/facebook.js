...
function Facebook(url) {
  return new Promise((resolve, reject) => {
    ...
    .then((response) => {
      ...
      const title = $('link[rel="alternate"]').last().attr('title') ? $('link[rel="alternate"]').last().attr('title').split('|') || $('title').text().split('|') : null;
      
      resolve({
        title: title ? title[0].trim() : null,
        upload_by: title && title[1] ? title[1].trim() || title[1].split('By')[1].trim() : null,
        link: {
          sd: parseString(sd[1]),
